...
//esse metodo pode estar errado pois não está combinando com o buscarTodos do modeloDAO
	public void mostrarNotebooks() {
		
		for (int i = 0; i < note.length; i++ ) {
			if (note[i] != null) {
				System.out.println(note[i].getSerialNote()+" ===== "+ 
									note[i].getDescricao()+ "=====" +
									note[i].getModelo() + "=====" +
									note[i].getDataCadastro()+ "=====" +
									note[i].getEstoque()+ "=====" +
									note[i].getFigura()+ "=====" +
									note[i].getPrecoUnitario());
			}
		}
		
	}

//os parametros do metodos atualizar podem acabar pracisando de um serialNote para saber onde atualizar
